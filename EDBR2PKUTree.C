...
#include "EDBR2PKUTree_headfile/resloved_boson.h"
#include "EDBR2PKUTree_headfile/resolved_boson_0AK8.h"
...
#include "EDBR2PKUTree_headfile/resolved_MJJlv_1AK8.h"
#include "EDBR2PKUTree_headfile/for_AK4_swtich_vector_to_error.h"
...
double dnn200[5] = {0.0,0.25,0.5,0.75,1.0};
	...
	for (Long64_t jentry=0; jentry<nentries;jentry++)
	{
        ...
        ForAK4_switchtoarray();
        // cout<<jentry<<"  : step 2 "<<endl;
    ...
    if(channelname=="el"){
        ...
        if(Common_genWeight>0) nn=1;
        else nn= -1;
        lumiWeight=Identical_lumiWeight/Nevent;
...
//                std::cout<<IDweight<<"        "<<btagweight_center<<"   "<<trigger_eff<<std::endl;
        ...
        weight=lumiWeight*Common_event_lepSF*ToppTweight;
        // cout<<"triggerWeight"<<triggerWeight<<"eff_and_pu_Weight"<<eff_and_pu_Weight<<"trigger_eff"<<trigger_eff<<"IDweight"<<IDweight<<"IDweightISO"<<IDweightISO<<"IDweighttrk"<<IDweighttrk<<"ToppTweight"<<ToppTweight<<endl;
...
//final weight------
    ...
    use_w_mass_resolved();
    use_z_mass_resolved();
    use_w_mass_resolved_0AK8();
    use_z_mass_resolved_0AK8();
    */
    inv_mass();
    ...
    //resolved_MJJlv_cal();
		...
		tmp_categoryID_channel=-1;// -1 for el; 1 for mu
            ...
            if (abs(lep) ==13 && Process_1Lep2fatjet_HLT_IsoMu24>0){
                    ExTree->Fill();
            }
            // if (Process_1Lep2fatjet_PassOldID ==1){
            //         ExTree->Fill();
            // }
            // ExTree->Fill();
		}else{
